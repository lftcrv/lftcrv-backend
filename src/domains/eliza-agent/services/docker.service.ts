...
export class DockerService implements IDockerService, OnModuleInit {
    ...
    async onModuleInit() {
        ...
        const baseImage = images.find(img => img.RepoTags?.includes('eliza:latest'));

        if (!baseImage) {
            throw new Error('Base Eliza image not found. Please build it first using ./scripts/docker.sh build');
        }
    }
    ...
    async createContainer(config: Record<string, any>): Promise<string> {
        ...
        const characterPath = path.join(this.charactersPath, `${config.name}.character.json`);

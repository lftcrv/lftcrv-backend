...
describe('PriceService', () => {
  ...
  describe('getCurrentPrice', () => {
    ...
    it('should throw error on API failure', async () => {
      ...
      await expect(service.getCurrentPrice('BTC')).rejects.toThrow(errorMessage);
    });
  });
  ...
  describe('getHistoricalPrices', () => {
    ...
    it('should handle supported timeframes correctly', async () => {
      const supportedTimeframes: TimeFrame[] = ['1m', '3m', '5m', '15m', '30m', '1h'];
      
      for (const timeframe of supportedTimeframes) {
        mockedAxios.get.mockResolvedValueOnce(mockKlineResponse);

        const prices = await service.getHistoricalPrices('BTC', timeframe, { limit: 2 });
  ...
  describe('Helper methods', () => {
    const mockCandle = [1625097600000, "35000.5", "35100.75", "34900.25", "35050.5", "100.5"];
    const mockResponse = {
      data: {
        results: [mockCandle]
      }
